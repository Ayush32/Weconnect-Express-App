...
module.exports.generate_token_and_send_mail = async (req, res) => {
  ...
  try {
    ...
    let job = queue
      .create("send_reset_pass_mail", token)
      .save(function (error) {
        if (error) {
    ...
    console.log(
      "There was a problem in creating a token to reset users password!",
      error
    );
    return;
  }
...
module.exports.change_password = function (req, res) {
  ...
  Token.findOneAndUpdate(
    ...
    function (error, token) {
      ...
      if (error) {
        console.log(
          "Error in finding the token with given access_token string!",
          error
        );
        return;
      }
